...
def tensor2cpp(tensor:torch.tensor,dtype:torch.dtype)->str:
    out_tensor:list[list]=list(tensor.detach().numpy().tolist())
    return str(out_tensor).replace('[','{').replace(']','}')
...
def cpp_code(layers:list,folder)->str:
    init_layers=""
    set_values=""
    sets=""
    ...
    return cpp_tempalte.format(init_layers=init_layers,body=body,set_values=set_values)